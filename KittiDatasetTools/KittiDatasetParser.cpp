#include "KittiDatasetParser.h"
#include <thread>

namespace kitti_dataset_tools {
KittiDatasetParser::KittiDatasetParser(QObject *parent)
    : QObject(parent), is_playing_(false), is_data_ready_(false) {}

void KittiDatasetParser::setParserSettings(std::string path_to_oxts_folder, EKF_INS::EKF &ekf) {
    ekf_ = &ekf;
    path_to_oxts_folder_ = path_to_oxts_folder;
}
...
void KittiDatasetParser::initState() {
    if (!is_data_ready_) {
...
void KittiDatasetParser::startPlayingData() {
  if (!is_data_ready_) {
    std::cout << "Can't play. Data is not ready." << std::endl;
    return;
  }
